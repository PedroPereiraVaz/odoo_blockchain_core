...
try:
    import warnings
    # Suppress websockets.legacy deprecation warning coming from web3 dependencies
...
class BlockchainRegistryEntry(models.Model):
    ...
    def action_register(self):
        """ Mark for registration in queue """
        for record in self:
    ...
    def process_blockchain_queue(self):
        """ CRON: Pick pending registrations AND revocations and send to chain """
        if not Web3:
    ...
    def _submit_transaction(self, w3, contract, account, record, is_revocation, chain_id):
        ...
        try:
            # Nonce management: always get latest
            ...
            try:
                ...
                hash_bytes = record.content_hash.encode() # fallback
            ...
            txn = func.build_transaction({
                'chainId': chain_id,
                'gasPrice': w3.eth.gas_price, # Use current safe gas
    ...
    def check_transaction_receipts(self):
        """ CRON: Check submitted transactions (both Registrations and Revocations) """
        if not Web3: return
    ...
    def _check_single_receipt(self, w3, record, is_revocation):
        ...
        try:
            ...
            # Tx not found yet (pending in mempool)
            pass
    
    def action_verify_on_chain_manual(self):
        """ Checks 'verifyDocument' view function """
        self.ensure_one()
        ...
        try:
             ...
             # (isValid, issuerName, issuerTaxId, timestamp, issuerAddress)
             is_valid = result[0]
             
             # result[0] is isValid (which means NOT revoked). 
             # We should also check status match.
             
             msg = _("Chain Verification: %s") % ("VALID" if is_valid else "INVALID / REVOKED")
             if is_valid:
                 msg += f"\nIssuer: {result[1]} ({result[4]})"
                 if self.status != 'confirmed': 
                     self.status = 'confirmed' # Auto-fix
             else:
                 # If invalid, check if it was revoked
